...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    `;
    if (parseInt(recentCount.count) >= RATE_LIMIT) {
      return NextResponse.json({ error: 'Demasiados mensajes. Vuelve mañana.' }, { status: 429 });
    }
    ...
    const users = await sql`
      SELECT id FROM users WHERE share_link = ${share_link}
    `;
